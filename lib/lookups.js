...
async function fetchElectronicMediaGroups(client) {
  const soql =
    ...
    `'productDetailImage', 'productListImage', 'productSearchImage', 'attachment', ` +
    `'bannerImage', 'tileImage')`;
...
async function fetchCategoryIdsByExternalId(client, externalIds, paceMs = SOQL_PACE_MS) {
  const unique = Array.from(new Set(externalIds.filter((s) => s && s.trim() !== '')));
  const map = new Map();
  ...
  for (let i = 0; i < unique.length; i += PRODUCT2_BATCH_SIZE) {
    ...
    const soql = `SELECT Id, External_Id__c FROM ProductCategory WHERE External_Id__c IN (${inList})`;
    const records = await queryAll(client, soql);
    for (const r of records) {
      if (r.External_Id__c) map.set(r.External_Id__c, r.Id);
    }
...
async function fetchExistingContentByExternalId(client, workspaceId, externalIds, paceMs = SOQL_PACE_MS) {
  const unique = Array.from(new Set(externalIds.filter((v) => v && v.trim() !== '')));
  const map = new Map();

  for (let i = 0; i < unique.length; i += PRODUCT2_BATCH_SIZE) {
    const chunk = unique.slice(i, i + PRODUCT2_BATCH_SIZE);
    const inList = chunk.map((v) => `'${escapeSoqlValue(v)}'`).join(',');
    const soql =
      `SELECT Id, ExternalId FROM ManagedContent ` +
      `WHERE AuthoredManagedContentSpaceId = '${escapeSoqlValue(workspaceId)}' ` +
      `AND ExternalId IN (${inList})`;
    const records = await queryAll(client, soql);
    for (const r of records) {
...
module.exports = {
  ...
  fetchExistingContentByExternalId,
  queryAll,
};
