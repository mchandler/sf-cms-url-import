...
Options:
  ...
  --kind <product|category>  Import target (default: product)
...
const VALID_KINDS = new Set(['product', 'category']);
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    switch (a) {
      ...
      case '--kind':        out.kind = takeValue(argv, i++, a); break;
...
function resolveConfig(argv) {
  ...
  const cfg = {
    ...
    kind: cli.kind ?? fromFile.kind ?? 'product',
  ...
  if (!VALID_KINDS.has(cfg.kind)) {
    fail(`--kind must be one of: ${Array.from(VALID_KINDS).join(', ')} (got "${cfg.kind}")`);
  }
